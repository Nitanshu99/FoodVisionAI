...
class QwenLLM:
    ...
    def _load_model(self):
        ...
        try:
            if not paths.LLM_MODEL_PATH.exists():
                print(f"⚠️ LLM model not found at {paths.LLM_MODEL_PATH}")
                print("💡 Run: python download_llm.py")
                return

            print(f"🧠 Loading Qwen2.5-0.5B from {paths.LLM_MODEL_PATH}...")

            # Conservative settings for 0.5B model
            self.model = Llama(
                model_path=str(paths.LLM_MODEL_PATH),
